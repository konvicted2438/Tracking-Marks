...
def init_kalman_filter(x, y, radius):
    ...
    # Measurement noise
    kf.R = np.eye(3) * 4.0  # tighter measurement trust
    
    # Process noise
    kf.Q = np.eye(6) * 0.1
    kf.Q[3:, 3:] *= 0.02  # quicker velocity response
    
    # Initial state
    kf.x = np.array([[x], [y], [radius], [0], [0], [0]])
    
    # Initial covariance
    kf.P = np.eye(6) * 3.0   # leaner initial uncertainty
...
def detect_and_track_circles(video_path, output_path=None, show_video=True):
    """
    Detect and track red circles in a video file using Kalman filtering.
    """
    ...
    frame_count = 0
    expected_points = 9  # We expect 9 points (3x3 grid)
    ...
    while True:
        ...
        # Detect circles
        detected_circles = detect_circles(gaussian_blurred)
        
        # Prediction step for all existing trackers
        for tracker in trackers:
            tracker['kf'].predict()
        
        # If this is the first frame with detections and we have enough circles
        if not trackers and len(detected_circles) >= expected_points:
            # Take the first 9 circles and arrange them in a grid
            points_to_track = detected_circles[:expected_points]
...
def main():
    video_path = Path("test_video/sphere2.mp4")
    if not video_path.is_file():
        raise FileNotFoundError(f"Video missing: {video_path.resolve()}")
    output_path = 'tracked_sphere2.avi'

    # Detect and track circles
    trajectories, all_circles, frame_timestamps = detect_and_track_circles(
        video_path, output_path, show_video=True
    )
